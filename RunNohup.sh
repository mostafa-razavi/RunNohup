...
if [ "$lowerCaseYesOrNo" == "y" ] || [ "$lowerCaseYesOrNo" == "yes" ]; then
	read -p "Enter your email address or addresses: " email
...
if [ "$lowerCaseYesOrNo" == "y" ] || [ "$lowerCaseYesOrNo" == "yes" ]; then
	nohup bash -c "  startSeconds=\$(date +%s) && echo \$startSeconds && $mainCommand && endSeconds=\$(date +%s) && duration=\$(echo \(\$endSeconds-\$startSeconds\) | bc) && $notification $email \$duration " > $logFile 2>&1 &
